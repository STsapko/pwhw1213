...
from src.repository import uses_repo as user_repository
...
async def register(
    ...
    existing_user = await user_repository.repo_user_authentication_by_email(
        body.email, db=db
    )
    if existing_user:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Email already exists. Try to log in.",
        )
    ...
    user_response = UserCreationResponse(
        ...
        created_at=created_user.password,
        avatar=created_user.avatar,
        message="Your account create successfully. Check email to activate it.",
    )
    bg_task.add_task(
        send_email, created_user.email, created_user.username, request.base_url
    )
    return user_response


@auth_router.post(
    "/login", status_code=status.HTTP_200_OK, response_model=OnLoginResponse
)
async def login(
    ...
    if not user.is_activated:
        raise HTTPException(
            ...
            detail="Inactive user. (Email not confirmed.)",
        )
    ...
    access_token = await auth_service.create_refresh_token(
    ...
    user_data = UserDBScheme(
        username=user.username, email=user.email, avatar=user.avatar
    )
    return OnLoginResponse(
        user=user_data, access_token=access_token, refresh_token=refresh_token_
    )
...
async def refresh_token(
    ...
    if token != user.refresh_token:
        await user_repository.repo_update_refresh_token(
            user=user, new_refresh_token=None, db=db
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid refresh token."
        )
    access_token = await auth_service.create_access_token(data={"sub": email})
    refresh_token_ = await auth_service.create_access_token(data={"sub": email})
    await user_repository.repo_update_refresh_token(
        user=user, new_refresh_token=refresh_token_, db=db
    )
    user_data = UserDBScheme(
        username=user.username, email=user.email, avatar=user.avatar
    )
    return OnLoginResponse(
        user=user_data, access_token=access_token, refresh_token=refresh_token
    )


@auth_router.get("/email_confirmation/{token}")
async def email_confirmation(token: str, db: AsyncSession = Depends(get_db)):
    email = auth_service.get_mail_from_token(token)
    user = await user_repository.repo_user_authentication_by_email(email=email, db=db)
    if not user:
        raise HTTPException(
...
async def request_confirmation_email(
    ...
    user = await user_repository.repo_user_authentication_by_email(
...
async def reset_password(
    ...
    bg_tasks.add_task(
        send_email_for_reset_pswd, email, user.username, reset_token, request.base_url
    )
...
@auth_router.post(
    "/set_new_password/{token}", dependencies=[Depends(RateLimiter(times=1, hours=1))]
)
